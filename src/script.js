...
// =========================
// PROJECT FOLDER
// Vanilla JavaScript adaptation of the React Bits Folder interaction.
// =========================
(() => {
    const folder = document.getElementById("projectFolder");
    const info = document.getElementById("folderProjectInfo");

    if (!folder) return;

    const papers = Array.from(folder.querySelectorAll(".folder-paper"));

    const projects = [
        {
            title: "ASCSMS",
            type: "School Canteen Stock Management",
            description: "A mini capstone project for the Automated Stock Management System for the School Canteen of Philippine Christian University – Manila — MAY 2026.",
            tech: ["Java", "MySQL", "XML", "GitHub"],
            link: "https://github.com/akiraLIXT/ASCSMS"
        },
        {
            title: "Personal Portfolio",
            type: "Web Development",
            description: "A responsive personal portfolio featuring interactive UI, dark/light mode, animations, and project presentation.",
            tech: ["HTML", "CSS", "JavaScript", "GPT-5.6 Luna"],
            link: "https://github.com/arjanjexzer11-star/Portfolio-Website"
        },
        {
            title: "TCG Looping Showcase",
            type: "JavaScript",
            description: "A small group project about the three looping functions in JavaScript.",
            tech: ["HTML", "CSS", "JavaScript"],
            link: "https://github.com/arjanjexzer11-star/TCG-looping-showcase"
        }
    ];

    let offsets = papers.map(() => ({ x: 0, y: 0 }));

    function setInfo(project) {
        if (!info) return;

        info.querySelector("h3").textContent = project.title;

        const paragraph = info.querySelector("p");
        paragraph.textContent = project.description;

        const existingTech = info.querySelector(".folder-info-tech");
        if (existingTech) existingTech.remove();

        const existingLink = info.querySelector(".folder-info-link");
        if (existingLink) existingLink.remove();

        if (project.tech && project.tech.length) {
            const tech = document.createElement("div");
            tech.className = "folder-info-tech";
        ...
        if (project.link) {
            ...
            link.className = "folder-info-link";
    ...
    function toggleFolder() {
        ...
        folder.setAttribute(
            "aria-label",
            isOpen ? "Close project folder" : "Open project folder"
        );

        if (!isOpen) {
            offsets = papers.map(() => ({ x: 0, y: 0 }));
            ...
            if (info) {
                info.querySelector("h3").textContent = "My selected work";
                info.querySelector("p").textContent = "Open the folder to view the projects inside.";
                info.querySelector(".folder-info-tech")?.remove();
                info.querySelector(".folder-info-link")?.remove();
            }
        } else {
            setInfo(projects[0]);
        }
    ...
    folder.addEventListener("click", (event) => {
        if (event.target.closest(".folder-paper")) return;
        toggleFolder();
    });
    ...
    papers.forEach((paper, index) => {
        paper.addEventListener("pointermove", (event) => {
            ...
            paper.style.setProperty("--magnet-x", `${offsets[index].x}px`);
            paper.style.setProperty("--magnet-y", `${offsets[index].y}px`);
